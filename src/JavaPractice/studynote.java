 ...
 *   
 *  『代入演算子の注意点』
 *  	・int a=b=c=5; のように変数宣言の初期値には使えない。　×
 *  	・a=b*2=C=5;　のように途中に式がある場合も使えない。　×
 *  	・a=b++=C=5; 同上 　×
 *  	・a=b=c*2; 右端に式がある分にはセーフ 〇
 *  
 *  『複合演算子の注意点』
 *  	・変数宣言後の初期値は忘れずに
 *  		(ex)コンパイルエラーの例
 *  　　　　　　int a;  //宣言だけなので、aに何の値も入っていないため×　　int a = 10; など初期値が必須。
 *              a += 5;  //　a = a+5と計算されるが、a+5の計算ができない。
 *   
 */
